...
export default {
    ...
    uid() {
        ...
        return array.map(value => alphabet[value % radix] as any).join('');
    },
    hash(str) {
            ...
            do {
                if (treeWalker.currentNode.nodeValue !== '\n') {
                    if (treeWalker.currentNode === range.endContainer &&
                        ...
                        (treeWalker.currentNode as any).splitText(range.endOffset);
                    }
                    if (startOffset) {
                        treeWalker.currentNode = (treeWalker.currentNode as any).splitText(startOffset);
                        startOffset = 0;
                    }
